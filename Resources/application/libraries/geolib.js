...
(function() {
	...
	geolib.distance = 0;
		...
		geolib.getDistance = function(start, end, accuracy) {
		 ...
		 */
		geolib.isPointInside = function(latlng, coords) {
			...
			for(var c = false, i = -1, l = coords.length, j = l - 1; ++i < l; j = i) {
			   ...
			   && (latlng.latitude < (coords[j].latitude - coords[i].latitude) 
					* (latlng.longitude - coords[i].longitude) / (coords[j].longitude - coords[i].longitude) + coords[i].latitude)
			   && (c = !c);
		 ...
		 * @return		array		ordered array
		 */
		geolib.findNearest = function(latlng, coords, offset) {
		 ...
		 */
		geolib.useDecimal = function(value) {
		 ...
		 */
		geolib.decimal2sexagesimal = function(dec) {
		 ...
		 */
		geolib.sexagesimal2decimal = function(sexagesimal) {
		 ...
		 */
		geolib.isSexagesimal = function(value) {

			return sexagesimalPattern.test(value);

		};

		geolib.round = function(value, n) {
